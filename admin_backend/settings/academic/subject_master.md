# Subject Master Backend Specification

## Module Overview
API handlers for maintaining the Subject Master table for Academic Settings. Requires Admin/Super Admin authorization.

## API List
| Method | Endpoint | Description |
| - | - | - |
| GET | `/api/subject_master` | Retrieve all items |
| POST | `/api/subject_master` | Create new item |
| PUT | `/api/subject_master/{id}` | Fully update item |
| PATCH | `/api/subject_master/{id}/status` | Toggle status |
...
- `Subject Master` uniqueness within `branch_id`.
...
| - | - | - |
| BE_AS_subject_master_01 | Unauthorized `teacher` request | Returns 403 Forbidden |
| BE_AS_subject_master_02 | Duplicate name insertion | Returns 400 Bad Request |
| BE_AS_subject_master_03 | Valid item creation | Returns 201 Created |
