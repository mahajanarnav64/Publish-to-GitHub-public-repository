# Academic Session Backend Specification

## Module Overview
API handlers for maintaining the Academic Session table for Academic Settings. Requires Admin/Super Admin authorization.

## API List
| Method | Endpoint | Description |
| - | - | - |
| GET | `/api/academic_session` | Retrieve all items |
| POST | `/api/academic_session` | Create new item |
| PUT | `/api/academic_session/{id}` | Fully update item |
| PATCH | `/api/academic_session/{id}/status` | Toggle status |
...
- `Academic Session` uniqueness within `branch_id`.
...
| - | - | - |
| BE_AS_academic_session_01 | Unauthorized `teacher` request | Returns 403 Forbidden |
| BE_AS_academic_session_02 | Duplicate name insertion | Returns 400 Bad Request |
| BE_AS_academic_session_03 | Valid item creation | Returns 201 Created |
