# Teacher Allocation Backend Specification

## Module Overview
API handlers for maintaining the Teacher Allocation table for Academic Settings. Requires Admin/Super Admin authorization.

## API List
| Method | Endpoint | Description |
| - | - | - |
| GET | `/api/teacher_allocation` | Retrieve all items |
| POST | `/api/teacher_allocation` | Create new item |
| PUT | `/api/teacher_allocation/{id}` | Fully update item |
| PATCH | `/api/teacher_allocation/{id}/status` | Toggle status |
...
- `Teacher Allocation` uniqueness within `branch_id`.
...
| - | - | - |
| BE_AS_teacher_allocation_01 | Unauthorized `teacher` request | Returns 403 Forbidden |
| BE_AS_teacher_allocation_02 | Duplicate name insertion | Returns 400 Bad Request |
| BE_AS_teacher_allocation_03 | Valid item creation | Returns 201 Created |
