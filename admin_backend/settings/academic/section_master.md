# Section Master Backend Specification

## Module Overview
API handlers for maintaining the Section Master table for Academic Settings. Requires Admin/Super Admin authorization.

## API List
| Method | Endpoint | Description |
| - | - | - |
| GET | `/api/section_master` | Retrieve all items |
| POST | `/api/section_master` | Create new item |
| PUT | `/api/section_master/{id}` | Fully update item |
| PATCH | `/api/section_master/{id}/status` | Toggle status |
...
- `Section Master` uniqueness within `branch_id`.
...
| - | - | - |
| BE_AS_section_master_01 | Unauthorized `teacher` request | Returns 403 Forbidden |
| BE_AS_section_master_02 | Duplicate name insertion | Returns 400 Bad Request |
| BE_AS_section_master_03 | Valid item creation | Returns 201 Created |
