# Section Master Frontend Specification

## Feature Overview
Provides interface for managing Section Master configurations under Academic Settings. Restricted to Admins.

## Page Overview
- Resides as a section in `frontend/index.html` with `id="section-master-page"`.
- Activated when `activeTab === 'section_master'`.
...
| Data Grid | Tabular presentation of Section Master records. |
| Add/Edit Modal | Form for creating or mutating Section Master. |
...
## User Flow
1. User navigates to Academic Settings -> Clicks Section Master card.
2. The Section Master page is rendered.
3. User creates, views, updates, or toggles states for Section Master.
...
| - | - | - |
| `/api/section_master` | GET | Fetch list |
| `/api/section_master` | POST | Create record |
| `/api/section_master/{id}` | PUT | Update record |
| `/api/section_master/{id}/status` | PATCH | Update status |

## Request JSON Example
```json
{
  "name": "Example Payload for Section Master",
  "status": "Active"
}
```
...
## Acceptance Criteria
- User can navigate to Section Master via module card.
...
| - | - | - |
| FE_AS_section_master_01 | Verify form prevents empty submission | Form blocks with error |
| FE_AS_section_master_02 | Ensure successful fetch loads grid | Grid populated |
| FE_AS_section_master_03 | Test Back button behavior | Returns to `activeTab = 'academic'` |
