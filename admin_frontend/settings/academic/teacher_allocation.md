# Teacher Allocation Frontend Specification

## Feature Overview
Provides interface for managing Teacher Allocation configurations under Academic Settings. Restricted to Admins.

## Page Overview
- Resides as a section in `frontend/index.html` with `id="teacher-allocation-page"`.
- Activated when `activeTab === 'teacher_allocation'`.
...
| Data Grid | Tabular presentation of Teacher Allocation records. |
| Add/Edit Modal | Form for creating or mutating Teacher Allocation. |
...
## User Flow
1. User navigates to Academic Settings -> Clicks Teacher Allocation card.
2. The Teacher Allocation page is rendered.
3. User creates, views, updates, or toggles states for Teacher Allocation.
...
| - | - | - |
| `/api/teacher_allocation` | GET | Fetch list |
| `/api/teacher_allocation` | POST | Create record |
| `/api/teacher_allocation/{id}` | PUT | Update record |
| `/api/teacher_allocation/{id}/status` | PATCH | Update status |

## Request JSON Example
```json
{
  "name": "Example Payload for Teacher Allocation",
  "status": "Active"
}
```
...
## Acceptance Criteria
- User can navigate to Teacher Allocation via module card.
...
| - | - | - |
| FE_AS_teacher_allocation_01 | Verify form prevents empty submission | Form blocks with error |
| FE_AS_teacher_allocation_02 | Ensure successful fetch loads grid | Grid populated |
| FE_AS_teacher_allocation_03 | Test Back button behavior | Returns to `activeTab = 'academic'` |
