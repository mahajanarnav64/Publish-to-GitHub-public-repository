# Class Master Frontend Specification

## Feature Overview
Provides interface for managing Class Master configurations under Academic Settings. Restricted to Admins.

## Page Overview
- Resides as a section in `frontend/index.html` with `id="class-master-page"`.
- Activated when `activeTab === 'class_master'`.
...
| Data Grid | Tabular presentation of Class Master records. |
| Add/Edit Modal | Form for creating or mutating Class Master. |
...
## User Flow
1. User navigates to Academic Settings -> Clicks Class Master card.
2. The Class Master page is rendered.
3. User creates, views, updates, or toggles states for Class Master.
...
| - | - | - |
| `/api/class_master` | GET | Fetch list |
| `/api/class_master` | POST | Create record |
| `/api/class_master/{id}` | PUT | Update record |
| `/api/class_master/{id}/status` | PATCH | Update status |

## Request JSON Example
```json
{
  "name": "Example Payload for Class Master",
  "status": "Active"
}
```
...
## Acceptance Criteria
- User can navigate to Class Master via module card.
...
| - | - | - |
| FE_AS_class_master_01 | Verify form prevents empty submission | Form blocks with error |
| FE_AS_class_master_02 | Ensure successful fetch loads grid | Grid populated |
| FE_AS_class_master_03 | Test Back button behavior | Returns to `activeTab = 'academic'` |
