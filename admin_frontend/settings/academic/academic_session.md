# Academic Session Frontend Specification

## Feature Overview
Provides interface for managing Academic Session configurations under Academic Settings. Restricted to Admins.

## Page Overview
- Resides as a section in `frontend/index.html` with `id="academic-session-page"`.
- Activated when `activeTab === 'academic_session'`.
...
| Data Grid | Tabular presentation of Academic Session records. |
| Add/Edit Modal | Form for creating or mutating Academic Session. |
...
## User Flow
1. User navigates to Academic Settings -> Clicks Academic Session card.
2. The Academic Session page is rendered.
3. User creates, views, updates, or toggles states for Academic Session.
...
| - | - | - |
| `/api/academic_session` | GET | Fetch list |
| `/api/academic_session` | POST | Create record |
| `/api/academic_session/{id}` | PUT | Update record |
| `/api/academic_session/{id}/status` | PATCH | Update status |

## Request JSON Example
```json
{
  "name": "Example Payload for Academic Session",
  "status": "Active"
}
```
...
## Acceptance Criteria
- User can navigate to Academic Session via module card.
...
| - | - | - |
| FE_AS_academic_session_01 | Verify form prevents empty submission | Form blocks with error |
| FE_AS_academic_session_02 | Ensure successful fetch loads grid | Grid populated |
| FE_AS_academic_session_03 | Test Back button behavior | Returns to `activeTab = 'academic'` |
